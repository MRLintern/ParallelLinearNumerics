...
int main() {
	...
	*/
	...
	for (double rFactor {1.0}; rFactor <= 2.0 + 1e-6; rFactor += 0.1) {

		std::cout<<"\nRunning Parallel SOR Algorithm with Relaxation Factor = "<<rFactor<<"\n";

		PSOR psor(A, b, tolerance, maxIterations, rFactor);

		Eigen::VectorXd x = psor.solve();
	}
