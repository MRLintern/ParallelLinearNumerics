...
void LPSolver::logPlotConvergence(const std::vector<double>& residuals, const std::string& method, double relaxFactor) const {

    if (residuals.empty()) {

    	std::cerr<<"ERROR: No residuals found for "<<method<<"\n";
    ...
    oss<<std::fixed<<std::setprecision(1)<<relaxFactor;
    std::string filename { resultsDir + "/" + method + "_RelaxFact(" + oss.str() + ")_convergence.csv" };
...
void LPSolver::logPlotConvergence(const std::vector<double>& residuals, const std::string& method) const {
	...
	logPlotConvergence(residuals, method, relaxFactor_);
